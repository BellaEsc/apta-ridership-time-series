...
For the second part of our analysis, we will be taking the OCTA time series and adding a second variable, California GDP. To do this, we will fit an ARIMAX model (an ARIMA model but with an X-tra variable). 
...
Now we need to detrend and make any transformations necessary before starting our analysis..
...
Looking at the plots, de-trending is definitely needed.

```{r}
ca_gdp_dl <- diff(log(ca_gdp_ts))
octa_q_dl <- diff(log(octa_q_ts))
plot.ts(ca_gdp_d)
plot.ts(octa_q_d)
```

## Model Selection
Now that we have our data matched up, we can start investigating what kind of model we should use. Using the package `vars`, I will be
```{r}
x <- cbind(octa_q_ts, ca_gdp_ts)
```
